...
const STORAGE_KEY = 'dxt_whatsapp_btn_position';
const DRAG_THRESHOLD = 6;

export default function WhatsAppButton() {
  const phoneNumber = '917389806555';
  const message = encodeURIComponent(
    'Hi! I have a question about my order/products from DXT.'
  );
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${message}`;

  const buttonRef = useRef(null);
  const dragInfo = useRef({ dragging: false, moved: false, startX: 0, startY: 0, offsetX: 0, offsetY: 0 });

  const getDefaultPosition = () => ({
    x: 24,
    y: (typeof window !== 'undefined' ? window.innerHeight : 800) - 80,
  });
